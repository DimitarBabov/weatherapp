...
__all__ = ["GFS_DIR", "retrieve_file", "output_file", "urls_from_file"]


NAME_REMOVE = ("http://", "https://", "github.com/", "/raw/")
GFS_DIR = Path(__file__).parent
...
def output_file(url: str, GFS_DIR: Path = GFS_DIR):
    file_name = url.strip()
    for part in NAME_REMOVE:
        file_name = file_name.replace(part, '').strip().strip('/:').strip()
    return Path(GFS_DIR, re.sub(r"[^\-_\.\w\d]+", "_", file_name))


def retrieve_file(url: str, GFS_DIR: Path = GFS_DIR, wait: float = 5):
    path = output_file(url, GFS_DIR)
    if path.exists():
        print(f"Skipping {url} (already exists: {path})")
    else:
        GFS_DIR.mkdir(exist_ok=True, parents=True)
